...
class ECCA(nn.Module):
    def __init__(self, c_dim, reduction) -> None:
        super().__init__()
        self.body = nn.Sequential(
            nn.Conv2d(c_dim, c_dim, (1, 1), padding="same"),
            nn.GELU(),
...
def get_local_weights(residual, ksize, padding):
    ...
    Returns:
        Tensor: 包含了输入残差张量每个局部窗口的方差
                这些方差可以作为权重用于后续的计算，例如在注意力机制或特征增强中
    """
    pad = padding
    # 使用反射填充方法对张量进行填充，为了计算局部窗口时能够处理图像边缘的像素
    residual_pad = F.pad(residual, pad=[pad, pad, pad, pad], mode="reflect")
    # 分别在高度和宽度上提取局部窗口，ksize窗口大小，3步长，每次窗口移动3个像素
...
class ESA(nn.Module):
    def __init__(self, num_feat=50, conv=nn.Conv2d) -> None:
        super().__init__()
        f = num_feat // 4
    ...
    def forward(self, input):
        ...
        c1_0 = self.maxPooling_0(self.conv2_0(c1_))  # strided conv 3
        c1_1 = self.maxPooling_1(self.conv2_1(c1_))  # strided conv 5
        c1_2 = self.maxPooling_2(self.conv2_2(c1_))  # stride conv 7
...
class MDSA(nn.Module):
    def __init__(self, c_dim, conv) -> None:
        super().__init__()
...
class EADB(nn.Module):
    def __init__(self, in_channels, conv=nn.Conv2d):
        ...
        # ESA + CCA
        # self.esa = BSRN_ESA(in_channels, conv)
        # self.cca = CCALayer(in_channels, reduction=16)
...
class MDRN(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=56, num_block=8, num_out_ch=3, upscale=4,
    ...
    def forward(self, input):
        ...
        out = self.upsampler(self.c2(self.GELU(self.c1(torch.cat([out_B1, out_B2, out_B3, out_B4,
                                                                  out_B5, out_B6, out_B7, out_B8], dim=1)))) + out_fea) + self.mean
...
if __name__ == "__main__":
    model = MDRN().cuda() # 321,904  MDSA+ECCA
